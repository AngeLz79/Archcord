...
const BASE_URL =
    "https://github.com/ather-net/Installer/releases/latest/download/";
const INSTALLER_PATH_DARWIN =
    "ArchcordInstaller.app/Contents/MacOS/ArchcordInstaller";
...
function getFilename() {
    switch (process.platform) {
        case "win32":
            return "ArchcordInstallerCli.exe";
        case "darwin":
            return "ArchcordInstaller.MacOS.zip";
        case "linux":
            return "ArchcordInstallerCli-linux";
        default:
...
async function ensureBinary() {
    ...
    const outputFile =
        process.platform === "darwin"
            ? join(FILE_DIR, "ArchcordInstaller")
            : downloadName;
    ...
    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "Archcord (https://github.com/Vendicated/Archcord)",
            "If-None-Match": etag,
        },
    });
    ...
    if (process.platform === "darwin") {
        ...
        logAndRun(
            `sudo spctl --add '${outputFile}' --label "Archcord Installer"`
        );
